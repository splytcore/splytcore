(function () {
  ...
  AssetsController.$inject = ['$scope', '$state', '$window', 'Authentication', 'assetResolve', '$q'];

  function AssetsController ($scope, $state, $window, Authentication, asset, $q) {
    var vm = this;
    ...
    vm.releaseAsset = releaseAsset;


    console.log('assetId: ' + vm.asset._id)
    console.log('assetIdHex: ' + vm.web3.toHex(vm.asset._id))
    
    if (vm.asset._id) {
      vm.assetContract = vm.assetABI.at(vm.asset.address)
    }
    ...
    function releaseAsset() {
      ...
      vm.assetContract.releaseAsset.sendTransaction(vm.gas, (err, trxid) => {
        console.log('trxid: ' + trxid)
      })
    }
    ...
    function save(isValid) {
      ...
      function successCallback(res) {
        ...
        let assetId = vm.web3.toHex(res._id)
        $q((resolve, reject) => {                  
          vm.assetManagerContract.createAsset.sendTransaction(assetId, vm.gas, (err, trxid) => {
            console.log('trxid: ' + trxid)
            if (err) {
              reject(err)
            } else {
