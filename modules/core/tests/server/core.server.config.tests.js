...
describe('Configuration Tests:', function () {
  ...
  describe('Testing default seedDB', function () {
    ...
    it('should have seedDB configuration set for "regular" user', function() {
      ...
      should.exist(userFromSeedConfig.displayName);
    ...
    it('should not be a "regular" user to begin with', function(done) {
      User.find({ displayName: 'user' }, function(err, users) {
        should.not.exist(err);
        users.should.be.instanceof(Array).and.have.lengthOf(0);
        return done();
      });
    ...
    it('should seed ONLY the admin user account when NODE_ENV is set to "production"', function(done) {
      ...
      User.find({ displayName: adminFromSeedConfig.displayName }, function(err, users) {
        ...
        seed
          ...
          .then(function() {
            User.find({ displayName: adminFromSeedConfig.displayName }, function(err, users) {
              ...
              _admin.displayName.should.equal(adminFromSeedConfig.displayName);
        ...
        seed
          ...
          .then(function() {
            User.find({ displayName: adminFromSeedConfig.displayName }, function(err, users) {
              ...
              User.find({ displayName: userFromSeedConfig.displayName }, function(err, users) {

                should.not.exist(err);
                users.should.be.instanceof(Array).and.have.lengthOf(1);

                var _user = users.pop();
                _user.displayName.should.equal(userFromSeedConfig.displayName);
    ...
    it('should seed admin, and "regular" user accounts when NODE_ENV is set to "test" when they already exist', function (done) {
      ...
      _admin.save(function (err) {
        ...
        _user.save(function (err) {
          ...
          User.find({ displayName: { $in: [adminFromSeedConfig.displayName, userFromSeedConfig.displayName] } }, function (err, users) {
    ...
    it('should ONLY seed admin user account when NODE_ENV is set to "production" with custom admin', function(done) {
      ...
      User.find({ displayName: admin1.displayName }, function(err, users) {
        ...
        seed
          ...
          .then(function() {
            User.find({ displayName: admin1.displayName }, function(err, users) {
              ...
              var _admin = users.pop();
              _admin.displayName.should.equal(admin1.displayName);
        ...
        seed
          ...
          .then(function() {
            User.find({ displayName: admin1.displayName }, function(err, users) {
              ...
              User.find({ displayName: user1.displayName }, function(err, users) {

                should.not.exist(err);
                users.should.be.instanceof(Array).and.have.lengthOf(1);

                var _user = users.pop();
                _user.displayName.should.equal(user1.displayName);
    ...
    it('should NOT seed admin user account if it already exists when NODE_ENV is set to "production"', function(done) {
      ...
      _admin.save(function(err, user) {
        // There shouldn't be any errors
        should.not.exist(err);
        user.displayName.should.equal(adminFromSeedConfig.displayName);
        ...
        seed
          ...
          .catch(function (err) {
            should.exist(err);
            err.message.should.equal('Failed due to local account already exists: ' + adminFromSeedConfig.displayName);
    ...
    it('should NOT seed "regular" user account if missing email when NODE_ENV set to "test"', function (done) {
      ...
      seed
        ...
        .catch(function (err) {
          should.exist(err);
          err.message.should.equal('Failed to add local ' + user1.displayName);
